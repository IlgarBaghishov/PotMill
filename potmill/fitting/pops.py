def pops(
    ...
    if batch_ID is None:
        ...
        parts = [
            np.load(f"{features_directory}{bid}/{rcuts_str}/a.npy", mmap_mode="r")[
                :, feature_indices
            ]
    ...
    for fold in range(n_fold):
        ...
        fweights_train = 1.0 / np.maximum(3.0, np.fabs(b_train[force_selector_train]))
        ...
        fweights_test = 1.0 / np.maximum(3.0, np.fabs(b_test[force_selector_test]))
        ...
        train_e_rmse_weighted = np.sqrt(
            np.sum(np.multiply(eweights_train, train_residual[energy_selector_train]))
        )
        train_f_rmse_weighted = np.sqrt(
            np.sum(np.multiply(fweights_train, train_residual[force_selector_train]))
        )
        print(
            "Energy training RMSE is",
            np.sqrt(np.mean(train_residual[energy_selector_train])),
            flush=True,
        )
        print(
            "Force training RMSE is",
            np.sqrt(np.mean(train_residual[force_selector_train])),
            flush=True,
        )
        ...
        test_e_rmse_weighted = np.sqrt(
            np.sum(np.multiply(eweights_test, test_residual[energy_selector_test]))
        )
        test_f_rmse_weighted = np.sqrt(
            np.sum(np.multiply(fweights_test, test_residual[force_selector_test]))
        )
        print(
            "Energy testing RMSE is",
            np.sqrt(np.mean(test_residual[energy_selector_test])),
            flush=True,
        )
        print(
            "Force testing RMSE is",
            np.sqrt(np.mean(test_residual[force_selector_test])),
            flush=True,
        )
        ...
        with open("results.csv", "a") as file:
            ...
            results_line += f",{eweight:.3f},{train_e_rmse:.10f},{train_f_rmse:.10f},{test_e_rmse:.10f},{test_f_rmse:.10f},{train_e_rmse_weighted:.10f},{train_f_rmse_weighted:.10f},{test_e_rmse_weighted:.10f},{test_f_rmse_weighted:.10f},{train_e_std_mean:.10f},{train_f_std_mean:.10f},{test_e_std_mean:.10f},{test_f_std_mean:.10f}\n"
