...
def _match_combo(run, batch, knee):
    want = (round(float(knee["rcut0"]), 6),
            [int(knee["nmax1"]), int(knee["nmax2"])],
            [int(knee["lmax1"]), int(knee["lmax2"])],
            round(float(knee["eweight"]), 3))
    for c in R.list_combos(run["run_dir"], batch):
        if (round(c["rcut"], 6), c["nmaxes"], c["lmaxes"], round(c["eweight"], 3)) == want:
            return c
    raise ValueError(f"Knee combo {want} not found in fits/{batch} (stop, do not guess)")
...
def convergence(run, batch, knee, out):
    ...
    files = sorted(glob.glob(rd + "pareto-front/results_*.csv"),
...
def _ann(ax, r):
    ...
    ax.text(0.04, 0.92, e, transform=ax.transAxes, fontsize=8,
...
def parity(rec, dE_row, out, hp):
    ...
    for k, (m, lab, unit) in enumerate([(e, "Energy", "eV/atom"), (f, "Force", "eV/Å")]):
        ...
        hb = ax[k].hexbin(x, y, C=c, reduce_C_function=np.mean, gridsize=180,
        ...
        cb = fig.colorbar(hb, ax=ax[k]); cb.set_label("ΔE_form above min (eV/atom)", fontsize=8)
...
def error_window(wm, out, hp):
    ...
    fig.suptitle(f"Error vs energy window (config count in grey; trust where >100) — knee: {hp}",
...
def error_kT(bm, out, hp):
    ...
    fig.suptitle(f"Boltzmann-kT error (n_eff in grey; trust where n_eff>50) — knee: {hp}",
                 fontsize=11, fontweight="bold")
    fig.tight_layout(rect=[0, 0, 1, 0.95])
    fig.savefig(out, dpi=200, bbox_inches="tight")
    print(f"Saved: {out}")
...
def main():
    ...
    combo = _match_combo(run, batch, knee)
    ...
    print(f"  per-element refs (eV/atom): "
          + ", ".join(f"{k}={v:.2f}" for k, v in fe["refs"].items()))

    parity(rec, dE_row, out_dir + "parity.pdf", hp)

    dmax = float(np.quantile(dE_row, 0.99))
    windows = sorted(set(np.round(np.linspace(0.3, dmax, 28), 3)).union({0.5, 1.0, 2.0, dE_row.max() + 1e-6}))
