...
def snap_hyperparameters_to_string(hyperparameters, delimiter=" ", w_eweight=True):
    ...
    if w_eweight:
        return rcut_string + delimiter + twojmax_string + delimiter + f"{hyperparameters[2]:.1f}"
    else:
...
def create_eweight_range(middle_eweight, n_eweights):
    eweight_list = [
        middle_eweight * 2**i for i in range((1 - n_eweights) // 2, (n_eweights + 1) // 2)
    ]
    return eweight_list
...
def combined_ace_hyperparameters(config, w_eweight=True):
    ...
    if w_eweight:
        hyperparameters_list = [
            ...
            for rcut_list in rcut_range
            for nmax_list in nmax_range
            for lmax_list in lmax_range
            for eweight in eweight_range
        ]
    else:
        hyperparameters_list = [
            [rcut_list, nmax_list, lmax_list]
            for rcut_list in rcut_range
            for nmax_list in nmax_range
            for lmax_list in lmax_range
        ]
...
def combined_snap_hyperparameters(config, w_eweight=True):
    ...
    if w_eweight:
        hyperparameters_list = [
            ...
            for rcut_list in rcut_range
            for twojmax_list in twojmax_range
            for eweight in eweight_range
        ]
    else:
        hyperparameters_list = [
            [rcut_list, twojmax_list] for rcut_list in rcut_range for twojmax_list in twojmax_range
        ]
    return hyperparameters_list
